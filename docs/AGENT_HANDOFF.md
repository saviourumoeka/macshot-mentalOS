...
2. **Pick exactly one task at a time.** If `## Active` has more than
...
3. **Append a Progress log entry on every meaningful step.** Not just
   ...
   *what changed*, *what was learned*, and *what is left*. Include
   commit SHAs when relevant. Treat the Progress log as the only
   memory the next agent has — write so a stranger can read it cold
   and continue.
...
5. **Acceptance criteria are immutable once a task is `Active`.** If
...
6. **Commits and tasks stay aligned.** Reference the task ID in every
   commit message: `feat(TASK-003): add embedding provider`. This
   makes `git log --grep=TASK-003` a complete audit trail.

7. **Branching.** All task work happens on `feat/...`, `fix/...`, or
...
8. **When in doubt, ask the user.** Do not silently change task
